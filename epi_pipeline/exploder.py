...
def calc_stats(data):
   ...
   b = data.apply(lambda x: x[-1] - x[-2]).to_numpy()
   r = data.apply(lambda x: (x[-1] - x[-8]) / 7).to_numpy()
   q = data.apply(lambda x: (x[-14] - x[-22]) / 7).to_numpy()
   return pd.DataFrame([ a, b, r, q, r-q, 100*b/a ]).transpose()
...
loss_stat_keys = ["dead", "dead_numIncrease", "dead_rolling", "dead_rolling_14days_ago", "dead_rolling_14days_ago_diff", "dead_pctIncrease"]
...
all_data["_id"] = all_data["ID"] + all_data["date"]

for key in case_stat_keys + loss_stat_keys + per_pop_stat_keys:
    all_data.loc[:, key] = pd.to_numeric(all_data[key], errors="coerce", downcast="float")
