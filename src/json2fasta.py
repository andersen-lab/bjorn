...
def convert_to_fasta(input_json, output_prefix, gadm_fp):
    ...
    seqs_dict = {
        sample['covv_virus_name'].replace('hCoV-19/', '').replace(' ', ''):
    ...
    gadm = pd.read_csv(gadm_fp)
    print(f"Standardizing location information...")
    max_region_columns = df["location"].apply(
    ...
    # Puerto Rico
    df.loc[(df['division_normed']=='Puerto Rico'), 'country_normed'] = 'Puerto Rico'
    df.loc[(df['country_normed']=='Puerto Rico'), 'division_normed'] = df.loc[(df['country_normed']=='Puerto Rico'), 'location'].fillna('None')
    ...
    print(f'Metadata generated and saving to {output_metadata}')
    ...
    df.to_csv(output_metadata, sep='\t', index=False, compression='gzip')
    return 0
